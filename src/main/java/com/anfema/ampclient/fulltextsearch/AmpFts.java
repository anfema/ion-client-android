 ...
 * <p>
 * Accessable via {@link AmpClient#fts()}
 */
public class AmpFts
{
	...
	public AmpFts( AmpClient ampClient, AmpConfig config, Context context )
	{
	...
	/**
	 * download full text search database for current collection
	 */
	public Observable<File> downloadSearchDatabase()
	{
		File dbTargetPath = DatabaseUtils.getPath( config.collectionIdentifier, context );
		Log.i( "FTS Database", "about to download FTS database for collection " + config.collectionIdentifier );
		return ampClient.getCollection()
	...
	/**
	 * Perform full text search. FTS database has to be downloaded via {@link AmpFts#downloadSearchDatabase()} first.
	 *
	 * @param searchTerm optional filter. can contain multiple terms separated by {@code " "}. no filter on text is applied if {@code null} or empty String
