...
import com.anfema.ionclient.caching.index.CollectionCacheIndex.Companion.save
import com.anfema.ionclient.caching.index.PageCacheIndex.Companion.retrieve
import com.anfema.ionclient.caching.index.PageCacheIndex.Companion.save
...
import com.anfema.ionclient.utils.IonLog.e
import com.anfema.ionclient.utils.IonLog.ex
import com.anfema.ionclient.utils.IonLog.i
import com.anfema.ionclient.utils.IonLog.w
import com.anfema.ionclient.utils.PagesFilter
import com.anfema.ionclient.utils.PendingDownloadHandler
import com.anfema.utils.NetworkUtils.isConnected
...
internal class IonPagesWithCaching(
    ...
    private val runningCollectionDownload //key: collection identifier
        : PendingDownloadHandler<String, Collection>
    private val runningPageDownloads //key: page identifier
        : PendingDownloadHandler<String, Page>
    ...
    override fun fetchCollection(preferNetwork: Boolean): Single<Collection> {
        val cacheIndex = retrieve(config, context)
        val currentCacheEntry = cacheIndex != null && !cacheIndex.isOutdated(config)
        val networkAvailable = isConnected(context) && cachingStrategy !== CachingStrategy.STRICT_OFFLINE
    ...
    override fun fetchPage(pageIdentifier: String): Single<Page> {
        ...
        return fetchCollection() // get page's last_changed date from collection
            ...
            .flatMap { isOutdated: Boolean? ->
                ...
                if (!isOutdated!!) {
                    ...
                    return@flatMap fetchPageFromCache(pageIdentifier, networkAvailable)
                } else if (networkAvailable) {
                    // download page
                    return@flatMap fetchPageFromServer(pageIdentifier)
                } else {
                    // no network available, but an old cached version exists
                    return@flatMap fetchPageFromCache(pageIdentifier, false)
                }
            }
    }
     ...
     * Use collection identifier as specified in [this.config]
     */
    ...
    private fun fetchCollectionFromCache(
        ...
        if (collection != null) {
            i("Memory Cache Lookup", collectionUrl)
            return Single.just(collection)
        }

        // retrieve from file cache
        i("File Cache Lookup", collectionUrl)
    ...
    private fun handleUnsuccessfulCollectionCacheReading(
        ...
        return if (serverCallAsBackup) {
            w("Backup Request", "Cache lookup $collectionUrl failed. Trying network request instead...")
            ex("Cache Lookup", e)
            fetchCollectionFromServer(cacheIndex)
        } else {
            e("Failed Request", "Cache lookup $collectionUrl failed.")
     ...
     * Uses default collection identifier as specified in [this.config]
     */
    private fun fetchCollectionFromServer(cacheIndex: CollectionCacheIndex?): Single<Collection> {
        ...
        val collectionSingle = ionApi.getCollection(
            config.collectionIdentifier, config.locale, config.variation, lastModified)
            .flatMap { serverResponse: Response<CollectionResponse> ->
                if (serverResponse.code() == COLLECTION_NOT_MODIFIED) {
                    ...
                    return@flatMap fetchCollectionFromCache(cacheIndex,
                    ...
                    return@flatMap Single.just(serverResponse) // unwrap page and remember byte count
                        ...
                        .doOnSuccess { collection: Collection? ->
                            MemoryCache.saveCollection(collection,
                                config,
                                context)
                        }
                    ...
                    return@flatMap Single.error<Collection>(HttpException(serverResponse))
                }
            }
            .onErrorResumeNext { throwable: Throwable? ->
                val collectionUrl = IonPageUrls.getCollectionUrl(config)
                e("Failed Request", "Network request $collectionUrl failed.")
    ...
    private fun fetchPageFromCache(pageIdentifier: String, serverCallAsBackup: Boolean): Single<Page> {
        ...
        if (memPage != null) {
            i("Memory Cache Lookup", pageUrl)
            return Single.just(memPage)
        }

        // retrieve from file cache
        i("File Cache Lookup", pageUrl)
    ...
    private fun handleUnsuccessfulPageCacheReading(
        ...
        if (serverCallAsBackup) {
            w("Backup Request", "Cache lookup $pageUrl failed. Trying network request instead...")
            ex("Cache Lookup", e)
            return fetchPageFromServer(pageIdentifier)
        }
        e("Failed Request", "Cache lookup $pageUrl failed.")
        return Single.error(ReadFromCacheException(pageUrl, e))
    }

    private fun fetchPageFromServer(pageIdentifier: String): Single<Page> {
        val pageSingle = ionApi.getPage(config.collectionIdentifier,
            pageIdentifier,
            config.locale,
            config.variation) // unwrap page and remember byte count
            ...
            .onErrorResumeNext { throwable: Throwable? ->
                val pageUrl = IonPageUrls.getPageUrl(config, pageIdentifier)
                e("Failed Request", "Network request $pageUrl failed.")
    ...
    private fun saveCollectionCacheIndex(lastModified: String?, lastUpdated: DateTime): Consumer<Collection> {
        return Consumer { collection: Collection? ->
            save(
                config, context, lastModified, lastUpdated)
        }
    }

    private fun savePageCacheIndex(): Consumer<Page> {
